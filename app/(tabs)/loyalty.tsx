...
export default function LoyaltyScreen() {
  ...
  const currentConfig = tierConfigs?.find((c) => c.tierName === loyalty?.tierName);
  const nextConfig = tierConfigs?.find((c) => c.tierName === nextTier);
        ...
        {loyalty && (
          <>
            ...
            <Animated.View
              ...
              style={{ backgroundColor: Colors.primary, borderRadius: 24, padding: 24, marginBottom: 16 }}
            >
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <View>
                  <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{t('loyalty.current')}</Text>
                  ...
                  <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginTop: 2 }}>
                    {t('home.washCount', { n: loyalty.totalSuccessfulWashes })}
                  </Text>
                </View>
            ...
            {currentConfig && (
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  ...
                  <Text style={{ fontSize: 15, fontWeight: '700', color: Colors.textPrimary }}>{t('loyalty.benefits')}</Text>
                  <TierBadge tier={loyalty.tierName} size="sm" />
                </View>
                {[
                  { label: t('loyalty.discount'),      value: `${currentConfig.discountPercent}%` },
                  { label: t('loyalty.pointsPer1k'),   value: `${currentConfig.pointsPer1000Vnd} ${t('loyalty.pointsUnit')}` },
                  { label: t('loyalty.bookingWindow'), value: `${currentConfig.bookingWindowDays} ${t('loyalty.days')}` },
                ].map((item) => (
            ...
            <Animated.View entering={FadeInDown.delay(160).springify()}>
              ...
              >
                <Text style={{ flex: 1, fontSize: 14, fontWeight: '500', color: Colors.textPrimary }}>{t('loyalty.historyTitle')}</Text>
